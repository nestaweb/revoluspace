...
const NavBar: React.FC<INavBarProps> = ({ current, locale }) => {
	...
	const linkColor = "block py-2 px-3 md:p-0 text-brand-gray-500 rounded md:hover:text-brand-violet-700 md:dark:hover:text-violet-500 dark:text-brand-white dark:hover:text-brand-white dark:border-gray-700 ease-in-out duration-200";
	...
	return (
		<nav className="border-gray-200 absolute w-full top-0 left-0 right-0 py-4">
			<div className="flex flex-wrap items-center justify-between mx-auto p-4">
				<Link href={`/${locale}`} className="flex items-center space-x-3 rtl:space-x-reverse relative z-40">
					<Image 
						src="/logo-text.svg" 
						width={500}
						height={500} 
				...
				<div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
					<ul className="flex flex-col text-brand-gray-500 font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 dark:border-gray-700">
						<li>

...
const NavBar: React.FC<INavBarProps> = ({ current }) => {
			...
			<div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
				...
				<div className="flex md:order-2 gap-x-4 w-[40vw] lg:w-[20vw] justify-end lg:justify-center items-center">
					<p className='flex gap-2 relative z-40'>
						<button className='font-extralight hover:font-semibold ease-in-out duration-200'>Fr</button>
						|
						<button className='font-semibold ease-in-out duration-200'>En</button>
					</p>
					{
						!isPhone ?
						<Link href="/contact">
							<button type="button" className="text-brand-white bg-brand-violet-700 hover:bg-violet-800 focus:outline-none font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-violet-600 dark:hover:bg-brand-violet-700 ease-in-out duration-200">Book a demo</button>
						</Link>
						:
						<>
							...
							<div className={`${isMenuOpen ? "right-0" : "-right-[100%]"} fixed max-h-screen w-screen h-screen max-w-screen top-0 bg-brand-white dark:bg-gray-800 md:hidden ease-linear duration-200 z-30`} id="navbar-cta">
								<ul className="flex absolute text-xl top-0 left-0 flex-col items-center justify-center h-full w-full m-0 font-semibold gap-y-8 z-30">
									<li>
										<Link href="/" className={current == "home" ? currentColor : linkColor}>Home</Link>
									</li>
									<li>
										<Link href="/slack"  className={current == "slack" ? currentColor : linkColor}>Slack</Link>
									</li>
									<li>
										<Link href="/msteams"  className={current == "msteams" ? currentColor : linkColor}>MS Teams</Link>
									</li>
									<li>
										<Link href="/pricing"  className={current == "pricing" ? currentColor : linkColor}>Pricing</Link>
									</li>
								</ul>
							</div>
						</>
